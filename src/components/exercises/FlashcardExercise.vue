...
//
// Nothing else holds that loop. The drill used to pause on a resolved card to
// show what there is to say about its word (#586), which made every card with a
// fact cost a Next — so the facts panel is gone from here (#690). It stays on
// the slower drills (Type, WordBank, Inflect, Speak, PhraseFix) and on the
// Dictionary, where there is already time to read it; a flashcard's job is to
// come round again quickly.
//
...
// the only thing left to do is move on. A correct answer never lands here — it
// advances straight to the next card.
...
<template>
  <div class="grid flashcard" style="gap: 1rem">
    ...
    <!-- The learning moment: the correct answer after a wrong guess or a pass.
         A right answer is already gone by the time this would render. -->
