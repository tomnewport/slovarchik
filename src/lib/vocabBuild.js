...
function normalizeWord(pos, key, word) {
  ...
  return {
    ...
    mannerPair: null,
    // The curriculum word this entry is an inflected form of. Set only on
    // gloss-only stubs, whose keys are surface forms rather than lemmas
    // («купи́=buy» is a form of «купи́ть=to buy»), and generated by
    // `scripts/gen-lemma-links.mjs`. Without it the two read as rival words,
    // which is what made phrase alignment refuse to credit either (#706).
  ...
  for (const w of learnableWords(words)) {
    for (const ex of w.usage ?? []) {
      ...
      const phrase = {
        ...
        cefr: w.cefr,
        enNotes,
        // Word-alignment inputs (#706), carried rather than resolved: which word
        // a token is gets worked out per phrase, when something actually asks.
        // `align` is the authored tie-break for tokens no rule settles;
        // `inflectToken` is the example's own claim that this token is `source`,
        // which `phrasesData.test.js` already checks against the paradigm and
        // which alignment gets for free.
        align: normalizeAlign(ex?.align),
        inflectToken: Number(ex?.inflect?.token) || null,
      }
      out.push(phrase)
    }
  }
 ...
 * @property {Object<number, string>} align authored token → word key tie-breaks,
 *   1-based like `inflect.token`; empty for the overwhelming majority
 * @property {number|null} inflectToken 1-based index of the `inflect:` target,
 *   whose word is `source`
 */

/**
 * An example's `align:` block as a plain 1-based index → word key map.
 *
 * YAML hands back string keys (`{ 3: "что=that" }` parses as `{"3": …}`), which
 * read back fine through numeric property access — but they are normalised here
 * anyway so a malformed entry is dropped at the door rather than surfacing as a
 * token that mysteriously refuses to align. `check:align` reports the drop.
 *
 * @param {unknown} align
...
function normalizeAlign(align) {
  ...
  const out = {}
  for (const [index, key] of Object.entries(align)) {
    const token = Number(index)
    if (!Number.isInteger(token) || token < 1) continue
    const word = String(key ?? '').trim()
    if (word) out[token] = word
  }
  return out
}
