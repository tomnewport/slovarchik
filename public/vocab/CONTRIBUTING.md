...
> stress or a ё does not hide it), a numeral's family (девя́тый, девятна́дцать and
> девяно́сто all lead back to де́вять — derived from `type` + `value`, so it is
> the numbers that match, not the letters), heteronyms (same spelling, different
> stress) and same-gloss words are already linked by `vocabBuild.js` and are
> picked up by the word panels for free. `confusable_with:` is for the
...
| `derived` | no facts, but the panel still has something — an aspect partner, a motion partner, the verb behind a participle, the adjective behind an adverb, a numeral's family, a heteronym, a same-gloss word |
