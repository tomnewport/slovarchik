 ...
 *
 * `npm test` green is not CI green (#655): six corpus gates —
 * `verify:review`, `check:inflect:cases`, `check:prompts`, `check:parts`,
 * `check:align`, `check:reader` — run only in
 ...
 *   node scripts/check-ci.mjs --corpus   # the six corpus gates only (~20s)
...
export const STEPS = [
  ...
  },
  {
    script: 'check:align',
    corpus: true,
    why: 'every phrase token resolves to one word, and the unresolved residue has not grown',
  },
  {
    script: 'check:reader',
    corpus: true,
    why: 'every tappable word in the shipped books has a dictionary entry',
  },
