...
src/
  ...
  views/*.vue           # one screen per route (HomeView + SessionView are the big ones;
                        #   BombDisposalView and InflectionCrushView are the minigames —
                        #   #726's between-practice interludes, launched for now from
                        #   HomeView's Minigames section rather than from inside a session)
